...
pub enum StandardBehavior {
    ...
    FollowTarget { target: VecN, speed: f64 },
...
impl AgentFactory {
    ...
    pub fn create_follower<S: AgentState + Default>(
        target: VecN,
        speed: f64,
    ) -> Box<dyn Agent<State = S>> {
        Box::new(BasicAgent::new(
            S::default(),
